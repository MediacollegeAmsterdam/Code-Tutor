...
export class AssignmentFeedbackCommand implements ICommand {
    ...
    async execute(
        ...
        const storedAction = context.extensionContext.globalState.get<{
            action: string;
            assignmentId: string;
            timestamp: string
            ...
            for (const [assignmentId, progress] of assignmentEntries) {
                ...
                if (fs.existsSync(filePath)) {
                    ...
                    if (startedAt) {
                        ...
                        stream.markdown(`| Gestart | 📅 ${startDate.toLocaleDateString('nl-NL')} om ${startDate.toLocaleTimeString('nl-NL', {
                            hour: '2-digit',
                            minute: '2-digit'
                        })} |\n`);
                    }
                    if (completedAt) {
                        ...
                        stream.markdown(`| Voltooid | 📅 ${completeDate.toLocaleDateString('nl-NL')} om ${completeDate.toLocaleTimeString('nl-NL', {
                            hour: '2-digit',
                            minute: '2-digit'
                        })} |\n`);
                    ...
                    if (gradedAt) {
                        ...
                        stream.markdown(`| Beoordeeld | 📅 ${gradeDate.toLocaleDateString('nl-NL')} om ${gradeDate.toLocaleTimeString('nl-NL', {
                            hour: '2-digit',
                            minute: '2-digit'
                        })} |\n`);
                    }
                    ...
                    if (highlight && highlight.trim().length > 0) {
                        ...
                        if (looksLikeCode) {
                            ...
                            let lang = 'text';
                            if (/\b(function|const|let|var|=>|async|await)\b/.test(highlight)) {
                                lang = 'javascript';
                            } else if (/\bdef\s+\w+|import\s+\w+|print\(/.test(highlight)) {
                                lang = 'python';
                            } else if (/\b(public|private|class|void|int|String)\b/.test(highlight)) {
                                lang = 'java';
                            } else if (/\b(using|namespace|Console\.)\b/.test(highlight)) {
                                lang = 'csharp';
                            }
