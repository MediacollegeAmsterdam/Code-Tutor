...
export function calculateAchievements(progressData: Record<string, number>): string[] {
	...
	if (total >= ACHIEVEMENTS_THRESHOLDS.firstStep) achievements.push('👶 Eerste Stap');
	if (total >= ACHIEVEMENTS_THRESHOLDS.activeStudent) achievements.push('🥉 Actieve Student');
	if (total >= ACHIEVEMENTS_THRESHOLDS.advancedStudent) achievements.push('🥈 Gevorderde Leerling');
	if (total >= ACHIEVEMENTS_THRESHOLDS.codeMaster) achievements.push('🏆 Code Meester');
	if ((progressData['debug'] || 0) >= ACHIEVEMENTS_THRESHOLDS.bugHunter) achievements.push('🐛 Bug Hunter');
	if ((progressData['review'] || 0) >= ACHIEVEMENTS_THRESHOLDS.codeReviewer) achievements.push('👀 Code Reviewer');
	if ((progressData['explain'] || 0) >= ACHIEVEMENTS_THRESHOLDS.eternalLearner) achievements.push('📚 Eeuwige Leerling');
	if ((progressData['quiz'] || 0) >= ACHIEVEMENTS_THRESHOLDS.quizMaster) achievements.push('🧠 Quiz Master');
	if ((progressData['exercise'] || 0) >= ACHIEVEMENTS_THRESHOLDS.exerciser) achievements.push('💪 Oefenaar');
